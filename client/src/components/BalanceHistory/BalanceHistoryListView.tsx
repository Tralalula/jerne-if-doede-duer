...
import BalanceHistoryFilters from './BalanceHistoryFilters';
import PageInfoDisplay from '../Pagination/PageInfoDisplay';
import PageSizeSelector from '../Pagination/PageSizeSelector';
import Pagination from '../Pagination/Pagination';

interface BalanceHistoryListViewProps {
    isAdmin?: boolean;
    userId?: string;
    showUserEmail?: boolean; 
}

export default function BalanceHistoryListView({ isAdmin = false, userId, showUserEmail = false }: BalanceHistoryListViewProps) {
                ...
                <Flex direction="column" gap="2">
                    ...
                    {!loading && entries.length > 0 && (
                        sortedEntries.map(entry => (
                            <BalanceHistoryCard key={entry.id} entry={entry} showUserEmail={showUserEmail} />
                        ))
                    )}
                </Flex>
                    ...
                    <Flex direction="column" gap="4" className="p-4">
                        ...
                        {!loading && entries.length > 0 && (
                            <div className="w-full min-w-[600px] overflow-x-auto">
                                <BalanceHistoryTable entries={sortedEntries} showUserEmail={showUserEmail} />
                            </div>
                        )}
